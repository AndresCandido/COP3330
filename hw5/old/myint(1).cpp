...
MyInt operator/ (const MyInt& x, const MyInt& y) {
    // if the divisor is zero, return an empty MyInt object
    if (y == MyInt(0)) {
        return MyInt();
    }
    // if the dividend is less than the divisor, return 0
    if (x < y) {
        return MyInt();
    }
    // allocate memory for the result MyInt object
    MyInt result;
    result.digits = new int[x.numDigits];
    result.numDigits = x.numDigits;
    ...
    MyInt dividend;
    int idx = x.numDigits - 1;
    while (idx >= 0) {
        dividend.digits = new int[idx + 1];
        dividend.numDigits = idx + 1;
        // copy the current digits from the dividend
        for (int i = 0; i <= idx; i++) {
            dividend.digits[i] = x.digits[i];
        }
        // perform a trial division to find the next digit of the quotient
        int q = 0;
        while (dividend >= y) {
            dividend = dividend - y;
            q++;
        ...
        while (dividend.numDigits > 1 && dividend.digits[dividend.numDigits - 1] == 0) {
            dividend.numDigits--;
        }
        idx--;
...
MyInt operator% (const MyInt& x, const MyInt& y) {
    ...
    while (idx >= 0) {
        ...
        // copy the current digits from the dividend
        for (int i = 0; i <= idx; i++) {
            dividend.digits[i] = result.digits[i];
        }
        // perform a trial division to remove the largest multiple of y from the dividend
        while (dividend >= y) {
            dividend = dividend - y;
        }
        // remove any leading zeros from the remainder
