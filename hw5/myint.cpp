...
MyInt operator/ (const MyInt& x, const MyInt& y) {
    ...
    // Use a loop to iterate through each digit of the dividend from left to right.
    for (int i = x.numDigits - 1; i >= 0; i--) {
        // Multiply the remainder by 10 and add the current digit of the dividend.
        remainder = remainder * 10;
        remainder = remainder + x.digits[i];

        // If the remainder is greater than or equal to the divisor, divide the remainder by the divisor
        // and add the quotient with the result. Otherwise, add a 0 to the quotient.
        if (remainder >= y) {
            int q = 0;
            MyInt r;
    ...
    // Return the quotient as the result of the division.
    return quotient;
}
...
MyInt operator% (const MyInt& x, const MyInt& y) {
    ...
    // Return the remainder as the result of the modulo operation.
    return remainder;
}
